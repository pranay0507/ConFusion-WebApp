  ...
  passport.authenticate('local',(err,user,info)=>{
    ...
    req.logIn(user,(err)=>{
      if(err){
        res.statusCode = 401;
        res.setHeader('Content-Type', 'application/json');
        res.json({success: false, status: 'Login Unsuccesful!',err:"could not login user"});
      }
      var token = authenticate.getToken({_id: req.user._id});
        res.statusCode = 200;
        res.setHeader('Content-Type', 'application/json');
        res.json({success: false, status: 'Login suuccesful!',token:token});
    });
  })(req,res,next);
});
...
router.get('/checkJWTToken',cors.corsWithOptions,(res)=>{
